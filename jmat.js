...
//- jmat.js ~~
//
//  This library's principal author is Jonas Almeida. I have re-written large
//  chunks of it with the goal of allowing it to pass JSLint :-)
//
//                                                          ~~ SRW, 08 Oct 2012
...
(function () {
 ...
 // Declarations
    ...
    var abs, array2mat, array2str, arrayfun, bin2dec, cat, catArray, cEl,
        clone, clone2, cloneArray, cloneVector, colon, dimfun, ones, parse,
        stringify, zeros;
 ...
 // Definitions
    ...
    array2mat = function (x) {
     // to handle indexed arrays by converting them into two separate
    ...
    array2str = function (x, sp) {
     // convert array into a sp separated
        if (!sp) {
            sp = '\n';
        }
        var i, y;
    ...
    bin2dec = function (x) {
        var n = x.length;
        return x.split('').map(function (xi, i) {
            return xi * Math.pow(2, n - i - 1);
        }).reduce(function (a, b) {
            return a + b;
        });
    };
    ...
    cat = function (x, y) {
     // cat will work for matrices and objects
        x = stringify(x);
        y = stringify(y);
        return parse(x.slice(0, x.length - 1) + ',' + y.slice(1, y.length));
    };

    catArray = function (A) {
     // optimized for conCATenation of an array of numerically indexed arrays
     // this function was developed to adress memory issues of dealing with
     // large arrays, not performance
        var Ai, Astr, i;
        Ai = '';
        Astr = '[';
    ...
    clone2 = function (x) {
     // clone object that may have functional elements
        return parse(stringify(x));
    };

    cloneArray = function (A) {
        if (Array.isArray(A)) {
            return A.map(function (x) {
                return jmat.cloneArray(x);
            });
        }
        return A;
    };
    ...
    cloneVector = function (V) {
     // fastar than cloneArray if your array only has one dimension
    ...
    dimfun = function () {
     // first argument is the function, subsequent arguments specify dimensions
     //
     // @Jonas: I rewrote this one to work with ES5 Strict Mode ...
     //
        var args, fun, i, x, x0, z;
        ...
        if (args.length > 1) {
            ...
            if ((typeof args[1]) !== 'number') {
                for (i = 0; i < args[1].length; i += 1) {
                    x[i] = args[1][i];
                }
            } else {
             // note first argument is always fun
    ...
    ones = function () {
        return dimfun(function () {
            return 1;
        }, arguments);
    };

    parse = function (x) {
     // x is a stringified Object
     //
     // @Jonas: This function probably won't work in ES5 Strict Mode, but it
     // _might_ work if you use `JSON.parse` in the same way that you used
     // `JSON.stringify` inside your own `stringify` function.
     //
        /*jslint evil: true */
        eval('var res = ' + x);
        return res;
    ...
    stringify = function (x) {
     // extends JSON.stringify to work with both values and functions
        var i, v, y;
        y = (typeof x);
        switch (y) {
        case 'function':
 ...
 // ------------------------------------------------------------------------
        ...
        compress: function (uncompressed) {
         ...
         // this is new -- such that we are compressing JS objects, not strings
            uncompressed = this.stringify(uncompressed);
         // Build the dictionary.
            var c, dictionary, dictSize, i, result, w, wc;
            dictionary = {};
            dictSize = 256;
            result = [];
            w = '';
         // @Jonas: Should `256` in the following line be `dictSize`?
            for (i = 0; i < 256; i += 1) {
